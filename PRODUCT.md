...
- **Pe' Fratte** (the association's trekking community) has its own page at `/pe-fratte/`, a second Vite entry (`pe-fratte/index.html` → `src/pe-fratte/`) built with the Pe' Fratte design system (paper/forest/gold, stamp shadows, Big Shoulders + Figtree + Space Mono) and a CSS bundle fully separate from the home's amethyst system. Its uscite are the `EVENTI` entries with `gruppo: "Pf"` (optional `percorso` block: never fill trail data that the group hasn't announced); page copy, meeting point, rules and the gallery live in `PE_FRATTE` in `content.js`. Sign-ups happen only in the WhatsApp group: no forms. Participation terms (e.g. "Gratuita") are set per uscita in `percorso.partecipazione`, never as a general rule, because outings may become paid in the future. The gallery (`PE_FRATTE.galleria`, files in `public/pe-fratte/galleria/`) holds only real photos and short muted clips from the outings, grouped by uscita; photos ship as .jpg + .webp with metadata (GPS) stripped, and videos autoplay only while visible.
